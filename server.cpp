...
struct server_params
{
	...
	// If non-empty, server requires this API key for protected endpoints
    std::string api_key = "";
};
...
void whisper_print_usage(int /*argc*/, char ** argv, const whisper_params & params, const server_params& sparams) {
    ...
    fprintf(stderr, "  --api-key KEY,                 [%-7s] Require this API key for protected endpoints (env WHISPER_API_KEY if not provided)\n", sparams.api_key.c_str());	
	fprintf(stderr, "             (or set WHISPER_API_KEY env var)\n");
...
// Returns true if auth OK or not required. If returns false, it sets 401 response.
bool check_api_key(const Request &req, Response &res, const server_params &sparams) {
    // If no api key configured, don't require auth
    ...
    if (req.has_header("Authorization")) {
        ...
        if (auth.rfind(prefix, 0) == 0) { // starts with prefix
            provided = auth.substr(prefix.size());
        } else {
            // maybe user sent raw token in Authorization header without "Bearer "
            provided = auth;
        }
    }

    // fallback to X-API-KEY header
    if (provided.empty() && req.has_header("X-API-KEY")) {
        provided = req.get_header_value("X-API-KEY");
    }

    if (provided.empty() || provided != sparams.api_key) {
        res.status = 401;
...
int main(int argc, char ** argv) {
    ...
    if (whisper_params_parse(argc, argv, params, sparams) == false) {
	    ...
	    // If CLI didn't provide an API key, try environment variable WHISPER_API_KEY
    if (sparams.api_key.empty()) {
        const char *env_api = std::getenv("WHISPER_API_KEY");
        if (env_api && env_api[0] != '\0') {
            sparams.api_key = std::string(env_api);
        }
    }
    ...
    svr->Post(sparams.request_path + sparams.inference_path, [&](const Request &req, Response &res){	
		...
		if (!check_api_key(req, res, sparams)) {
			return;
		}
